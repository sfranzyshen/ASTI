...
std::string extractCppCommands(int testNumber) {
    ...
    try {
        ...
        std::cout << "ERROR in extraction: " << e.what() << std::endl;
        return "";
    }
}

int main() {
    std::cout << "Testing extraction only for test 114..." << std::endl;
    std::string result = extractCppCommands(114);
    std::cout << "Extraction result length: " << result.length() << std::endl;
    if (result.empty()) {
