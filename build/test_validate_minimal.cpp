...
int main() {
    std::cout << "=== Minimal Cross-Platform Validation (Test 114 Only) ===" << std::endl;

    // Extract both command streams
    std::cout << "Extracting C++ commands..." << std::endl;
    std::string cppCommands = extractCppCommands(114);
    std::cout << "Loading JS commands..." << std::endl;
    std::string jsCommands = loadJsCommands(114);

    // Compare functionally
    std::cout << "Comparing commands..." << std::endl;
    bool matches = compareCommands(cppCommands, jsCommands, 114);

    std::cout << "Result: " << (matches ? "MATCH" : "DIFFERENCE") << std::endl;
    return matches ? 0 : 1;
}