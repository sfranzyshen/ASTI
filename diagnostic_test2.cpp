...
int main() {
    std::cout << "=== DETAILED COMMAND COMPARISON - Blink.ino ===" << std::endl;
    
    // Load AnalogReadSerial example AST
    std::vector<uint8_t> astData;
    std::ifstream astFile("test_data/example_002.ast", std::ios::binary);
    if (!astFile) {
        std::cerr << "Failed to load example_002.ast" << std::endl;
        return 1;
    }
